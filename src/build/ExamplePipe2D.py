...
class ExamplePipe2D(BasePipe):
    ...
    def fit(self, x: np.ndarray, x_2d: np.ndarray, y: np.ndarray) -> None:
        self.log.debug("Fit estimator with feature shape %s and target shape %s", x_2d.shape, y.shape)
        self.__estimator.fit(x_2d, y)

    def predict(self, x: np.ndarray, x_2d: np.ndarray) -> np.ndarray:
        self.log.debug("Predict with estimator from feature shape %s", x_2d.shape)
        return self.__estimator.predict(x_2d)
...
def test():
    ...
    # estimator
    esti = ExamplePipe2D(work_dir=Path())
